...
Engine_Farrago : CroneEngine {
	...
	alloc {
		...
		def = SynthDef(\Farrago, {
			...
			// TODO: not too satisfied w/ the env range, tames things a bit too much
			// also kbd tracking works well for positive values but when negative is a bit borked
			// instantCutoff = (cutoff * (1 + (fktrack * freq2.cpsmidi / 12))).clip(20, 20000);
			instantCutoff = (cutoff2 * (1 + (fktrack * freq2.cpsmidi / 12)) + fenv.linlin(0, 1, 0, cutoff)).clip(20, 20000);
